...
> **文書状態: 計画（未実装の要件を含む）。** 本ドキュメントは、Microsoft VS Code の共通基盤層 (`src/vs/base/`) を Rust ネイティブ環境へ移植する場合の機能要件定義書です。要件の記載は実装完了を意味しません。現在提供する機能は[実装状況とロードマップ](../implementation-status.md)を参照してください。

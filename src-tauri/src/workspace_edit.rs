...
pub fn apply_plan(
    ...
    if let Err(error) = result {
        if let Err(rollback_error) = restore_backups(&backups) {
            return Err(format!(
                "Workspace edit failed: {error}. Rollback also failed: {rollback_error}"
            ));
        }
        return Err(format!(
            "Workspace edit failed and was rolled back: {error}"
        ));
    }
            ...
            let contents =
                if path.exists() {
                    Some(fs::read(&path).map_err(|_| {
                        format!("Could not create a backup of '{}'", path.display())
                    })?)
                } else {
                    None
                };
...
fn apply_operations(operations: &[ResolvedOperation]) -> Result<(), String> {
    for operation in operations {
        match operation {
            ResolvedOperation::Write { path, content, .. }
            | ResolvedOperation::Create { path, content, .. } => {
            ...
            ResolvedOperation::Rename { from, to, .. } => {
                ...
                fs::rename(from, to)
                    .map_err(|error| format!("Could not rename '{}': {error}", from.display()))?;
            }
        }
    }
    Ok(())
            ...
            None => {
                if backup.path.exists() {
                    fs::remove_file(&backup.path).map_err(|error| {
                        format!(
                            "Could not remove rollback artifact '{}': {error}",
                            backup.path.display()
                        )
                    })?;
                }
...
fn insert_unique_path(paths: &mut HashSet<PathBuf>, path: &Path) -> Result<(), String> {
    if !paths.insert(path.to_path_buf()) {
        return Err(format!(
            "Workspace edit contains multiple operations for '{}'",
            path.display()
        ));
    }
    Ok(())
...
fn ensure_regular_file(path: &Path, action: &str) -> Result<(), String> {
    let metadata = fs::metadata(path)
        .map_err(|_| format!("Cannot {action} '{}': file does not exist", path.display()))?;
    if !metadata.is_file() {
        return Err(format!(
            "Cannot {action} '{}': only regular files are supported",
            path.display()
        ));
    }
    Ok(())
...
fn ensure_writable(path: &Path) -> Result<(), String> {
    let metadata =
        fs::metadata(path).map_err(|_| format!("Could not inspect '{}'", path.display()))?;
    if metadata.permissions().readonly() {
        return Err(format!(
            "Cannot modify '{}': the file is read-only",
            path.display()
        ));
    }
    Ok(())
...
fn ensure_parent_writable(path: &Path) -> Result<(), String> {
    let parent = path
        .parent()
        .ok_or_else(|| "Workspace edit path has no parent".to_string())?;
    ...
    {
        return Err(format!(
            "Cannot modify '{}': parent directory is read-only",
            path.display()
        ));
    }
    Ok(())
...
fn nearest_existing_parent(path: &Path) -> Result<&Path, String> {
    ...
    while !current.exists() {
        current = current
            .parent()
            .ok_or_else(|| "Workspace edit path has no existing parent".to_string())?;
    }
    Ok(current)
}
...
fn ensure_expected_content(path: &Path, expected: Option<&str>) -> Result<(), String> {
    ...
    let actual = fs::read_to_string(path)
        .map_err(|_| format!("Cannot read '{}' for conflict detection", path.display()))?;
    if actual != expected {
        return Err(format!(
            "Cannot modify '{}': the file changed since the edit was previewed",
            path.display()
        ));
    }
    Ok(())
...
fn write_atomically(path: &Path, contents: &[u8]) -> Result<(), String> {
    ...
    let result = (|| -> Result<(), String> {
        let mut file = File::create(&temporary)
            .map_err(|_| "Could not create a temporary edit file".to_string())?;
        file.write_all(contents)
            .map_err(|_| "Could not write a temporary edit file".to_string())?;
        file.sync_all()
            .map_err(|_| "Could not flush a temporary edit file".to_string())?;
        drop(file);
        fs::rename(&temporary, path)
            .map_err(|_| format!("Could not replace '{}' atomically", path.display()))?;
        Ok(())
    })();
    if result.is_err() {
